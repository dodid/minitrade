...
def test_create_tradeplan(
    ...
    plan.save()
    assert TradePlan.get_plan('pytest_tradeplan') == plan


def test_run_backtest():
    plan = TradePlan.get_plan('pytest_tradeplan')
...
def test_trade(launch_ibgateway):
    trader = Trader()
    trader.execute()
